...
impl TestCluster {
    async fn bootstrap() -> Self {
        ...
        let socket = quic::Connector::new(authorized_client_keypair(0)).unwrap();
    ...
    async fn new_client_api_client(&self) -> client_api::Client<quic::Connector> {
    ...
    async fn new_storage_api_client(&self) -> storage_api::Client<quic::Connector> {
...
) -> client_api::Client<quic::Connector> {
    let mut config = client_api::client::Config::new([]);

    let keypair = f(&mut config);

    let socket = quic::Connector::new(keypair).unwrap();
