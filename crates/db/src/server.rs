    ...
    async fn execute<'a>(&'a self, op: Operation<'a>) -> storage_api::Result<Output<'a>> {
        ...
        let res = match op {
            ...
            Operation::Del(op) => self
                .string_storage()
                .del(&key(op.key), op.version.to_unix_timestamp_micros())
            ...
            Operation::HSet(op) => {
                ...
                self.map_storage()
                    .hset(&key(op.key), &pair, expiration, version)
            ...
            Operation::HGetExp(op) => self
                .map_storage()
                .hget_exp(&key(op.key), &op.field.into())
...
fn key(key: Bytes<'_>) -> GenericKey {
    let pos = std::hash::BuildHasher::hash_one(&HASHER, &key);
    GenericKey::new(pos, key.into_owned())
}
