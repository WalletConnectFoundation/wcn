use {
    ...
    futures::{stream, FutureExt, StreamExt as _},
...
struct NodeHandle {
    ...
    /// Coordinator API client to make coordinator requests _to_ node.
...
fn new_node_config() -> Config {
    ...
    Config {
        ...
        keypair,
        metrics_addr: format!("127.0.0.1:{}", NEXT_PORT.fetch_add(1, Ordering::Relaxed)),
...
//             .clone()
//             .spawn(async move {
//                 let dir: PathBuf = format!("/tmp/irn/test-node/{}",
// idt.peer_id)                     .parse()
//                     .unwrap();

//                 let cfg = Config {
//                     id: idt.peer_id,
//                     replica_api_server_addr: idt.addr,
//                     is_raft_member,
//                     known_peers: peers.clone(),
//                     bootstrap_nodes: is_bootstrap.then(|| peers.clone()),
//                     raft_dir: dir.join("raft"),
//                     keypair: idt.keypair.clone(),
//                     coordinator_api_server_addr: idt.api_addr.clone(),
...
//     async fn pre_bootup(&mut self, idt: &test::NodeIdentity, node:
// &test::Node<Self>) {         let node_ctx =
// self.nodes.get_mut(&idt.peer_id).unwrap();         let _guard =
// node_ctx.runtime.enter();         Network::spawn_servers(&node_ctx.config,
// node.clone(), None).unwrap();     }

//     async fn post_shutdown(&mut self, node: &test::NodeHandle<Self>, reason:
// Shutdown) {         let node_ctx = self.nodes.remove(node.id()).unwrap();
//         node_ctx
//             .runtime
...
//         // Seems like `openraft` resolves the shutdown future before
// finishing all of         // its spawned tasks. Not waiting here (sometimes!)
// causes it to fail to         // write the latest FSM state to disk.
//         tokio::time::sleep(Duration::from_secs(1)).await;
...
//         num_groups: 3,
//         node_opts: NodeOpts {
//             replication_strategy: Strategy::new(3,
// cluster::replication::ConsistencyLevel::Quorum),
// replication_request_timeout: Duration::from_secs(15),
// replication_concurrency_limit: 1000,             replication_request_queue:
// 4096,             warmup_delay: Duration::from_secs(1),
//             authorization: None,
//         },
//     })
//     .await;
...
//                 nodes: [(n.identity().peer_id.id,
// n.identity().api_addr.clone())]                     .into_iter()
//                     .collect(),
...
//     // Subscribers need to be polled in order to connect to the IRN backend,
// so we     // wait a bit before publishing.
//     tasks.push(
//         async {
...
//             // We should receive a timeout error, since there's no more
...
//         key: auth::client_key_from_secret(b"test").unwrap(),
//         nodes: [(node.identity().peer_id.id,
// node.identity().api_addr.clone())]             .into_iter()
//             .collect(),
//         shadowing_nodes: Default::default(),
//         shadowing_factor: Default::default(),
//         request_timeout: Duration::from_secs(5),
...
//     // Add shared data without a namespace. Validate that it's accessible by
// both     // clients.
//     client0
...
//     // Validate that the client returns an access denied error trying to
// access     // unauthorized namespace.
//     assert!(matches!(
...
//         // Validate `hget`.
//         for (field, value) in &data {
//             let data = client0.hget(key.clone(),
// field.0.clone()).await.unwrap();             assert_eq!(data,
// Some(value.0.clone()));         }

//         // Validate `hvals`.
//         assert_eq!(
//             crate::sort_data(client0.hvals(key.clone()).await.unwrap()),
//             crate::sort_data(data.iter().map(|v| v.1
// .0.clone()).collect::<Vec<_>>())         );
