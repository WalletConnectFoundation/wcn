...
pub use {
    wcn_rpc::{PeerAddr, PeerId, identity::Keypair},
...
pub enum Error {
    #[error("Bootstrap nodes connection failed")]
    NoAvailableNodes,

    #[error("Retries exhausted")]
    RetriesExhausted,

    #[error("Failed to create cluster: {0}")]
    ClusterCreation(#[from] wcn_cluster::view::CreationError),
...
impl Client {
    pub async fn new(config: Config) -> Result<Self, Error> {
        ...
        let bootstrap_cluster =
            cluster::Cluster::new::<Error>(cluster_cfg.clone(), bootstrap_sc).await?;
        ...
        let cluster = cluster::Cluster::new::<Error>(cluster_cfg, dynamic_sc).await?;
...
impl RequestExecutor for Client {
    ...
    where
        ...
        op::Output: op::DowncastOutput<T>,
    {
        let client_conn = self.using_next_node(|node| node.coordinator_conn.clone());

        async move { self.execute_request(&client_conn, op).await }
    }
}
    ...
    O: RequestObserver + Send + Sync,
{
        ...
        op::Output: op::DowncastOutput<T>,
    {
        ...
        let (conn, mut metadata) = self.using_next_node(|node| {
