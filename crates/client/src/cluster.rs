...
struct InvalidPeerAddr(PeerAddr);
...
async fn try_fetch_cluster_view(
    ...
    let sock_addr = peer_addr
        .quic_socketaddr()
        .map_err(|_| Error::internal(InvalidPeerAddr(peer_addr.clone())))?;

    let SocketAddr::V4(sock_addr) = sock_addr else {
        return Err(Error::internal(InvalidPeerAddr(peer_addr.clone())));
    };

    let view = client
        .connect(sock_addr, &peer_addr.id, ())
        .await
